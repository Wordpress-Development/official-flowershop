...
class backup_restore_theme_options {
	...
	function import_export() {
		...
		if (isset($_POST['upload']) && check_admin_referer('shapeSpace_restoreOptions', 'shapeSpace_restoreOptions')) {
			if ($_FILES["file"]["error"] > 0) {
				...
				$options = wp_remote_fopen($_FILES["file"]["tmp_name"]);	
	...
	function _get_options() {
		global $wpdb;
		return $wpdb->get_results("SELECT option_name, option_value FROM {$wpdb->options} WHERE option_name = 'theme_mods_official-flowershop'"); // edit 'shapeSpace_options' to match theme options
	}
}
